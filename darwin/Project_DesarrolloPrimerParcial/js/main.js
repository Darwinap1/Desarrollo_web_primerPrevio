...
function setupWantedBookForm() {
    const form = document.getElementById('wanted-book-form');
    const wantedList = document.getElementById('wanted-list');
    if (!form || !wantedList) return;
    ...
    form.addEventListener('submit', (event) => {
        ...
        const author = document.getElementById('wanted-author').value.trim();
        ...
        const request = { title, author: author || 'Desconocido' };
        wantedRequests.push(request);
        renderWantedRequests(wantedList, wantedRequests);

        form.reset();
    });
}

function renderWantedRequests(container, requests) {
    container.innerHTML = '';
    if (requests.length === 0) {
        container.innerHTML = '<p>Aún no hay solicitudes. Sé el primero en pedir un libro.</p>';
        return;
    }
    ...
    requests.forEach((req, idx) => {
        ...
        item.textContent = `${idx + 1}. ${req.title} - Autor: ${req.author}`;
